/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package projectadvancedoperasi;

/**
 *
 * @author Kaitoga
 */
public class Operasi {
    int jumlahkan(int a, int b){
        return (a+b);
    }
    
    int jumlahkan(int a, int b, int c){
        return (a+b+c);
    }
    
    double jumlahkan(double a, double b, double c){
        return (a+b+c);
    }
    
    int kali(int a, int b){
        return (a*b);
    }
    
    int kali(int a, int b, int c){
        return (a*b*c);
    }
    
    double kali(double a, double b){
        return (a*b);
    }
    
    double kali(double a, double b, double c){
        return (a*b*c);
    }
    
    
    double kali(int a, double b){
        return (a*b);
    }
    
    double kali(double a, int b){
        return (a*b);
    }
    
    double kali(int a, double b, double c){
        return (a*b*c);
    }
    
    double kali(double a, int b, double c){
        return (a*b*c);
    }
    
    double kali(double a, double b, int c){
        return (a*b*c);
    }
    
    double kali(int a, double b, int c){
        return (a*b*c);
    }
    
    double kali(double a, int b, int c){
        return (a*b*c);
    }
    
    double kali(int a, int b, double c){
        return (a*b*c);
    }
}
